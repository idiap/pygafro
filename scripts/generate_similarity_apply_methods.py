...
#include "algebra/similarity_transformation_utils.hpp"


void {function_name}(py::class_<gafro::SimilarityTransformation<double>, Multivector_scalare12e13e23e0ie1ie2ie012ie3ie013ie023ie123i> &c)
{{
"""
        )
        for mv in multivectors:
            output.write(f'    c.def("apply", &similarity_apply<{mv}>);\n')
...
for i in range(0, count, N):
    ...
    generate_file(
        os.path.join(sys.argv[1], f"similarity_apply_methods_{nb}.cpp"),
        mvs,
        f"init_similarity_apply_methods_{nb}",
    )
    nb += 1


generate_index_file(
    os.path.join(sys.argv[1], "similarity_apply_methods.cpp"),
    "init_similarity_apply_methods",
    nb,
)
...
if nb != 19:
    print(
        "The number of 'similarity apply methods' files has changed, update the 'scripts/CMakeLists.txt' file!"
    )
    sys.exit(1)
