...
#include "singlemanipulatordualtargets.h"
...
with open(os.path.join(sys.argv[1], "singlemanipulatordualtargets.h"), "w") as output:
    template = helpers.load_template("singlemanipulatordualtargets.h")
...
template = helpers.load_template("singlemanipulatordualtargets.hpp")
template_functions = helpers.load_template("singlemanipulatordualtargets_functions.hpp")

nb = 0
for i in range(0, count, N):
    generate_file(
        os.path.join(sys.argv[1], f"singlemanipulatordualtargets_{nb}.cpp"),
        f"init_singlemanipulatordualtargets_{nb}",
        template,
        template_functions,
        entries[i : i + N],
    )
...
generate_index_file(
    os.path.join(sys.argv[1], "singlemanipulatordualtargets.cpp"), "init_singlemanipulatordualtargets", nb
)


if nb != 61:
    print(
        "The number of generated 'singlemanipulatordualtargets' files has changed, update the 'scripts/CMakeLists.txt' file!"
    )
    sys.exit(1)
